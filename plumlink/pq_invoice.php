...
$request = exploderequest();
 ...
 for ($i = 0 ; $i < count($doc[6]);$i++) {
  ...
  list($w,$d) = numsizes($doc[9][$i]/$doc[8][$i]);
  $pdf->Text(162-$w,$ypos,$d);

  list($w,$d) = numsizes($doc[9][$i]/$doc[8][$i]*$doc[10][$i]);
 ...
 $filename = __DIR__.'/pdfdocs/INVOICE_'.$request['id'][0].'.PDF';
 echo $filename;
...
// $pdf->Text(15,287,'VAT Number: '.$request['company'][8].'   e:admin@plumlink.co.uk ');
