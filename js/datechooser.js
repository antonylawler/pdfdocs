...
function dateChooserSetDate(inputId, value) {
  ...
  if (input && input.DateChooser) {
    ...
    if (input.DateChooser.isTimeChooser()) {
      ...
      theForm.elements[prefix + 'hour'].selectedIndex].value) + parseInt(theForm.elements[prefix + 'ampm'].options[
      theForm.elements[prefix + 'ampm'].selectedIndex].value), parseInt(theForm.elements[prefix + 'min'].options[
...
function dateChooserDateChange(prefix) {
  var input = document.getElementById(document.getElementById(prefix + 'inputId').value);
  var newDate = new Date(
  document.getElementById(prefix + 'year').options[document.getElementById(prefix + 'year').selectedIndex].value,
  document.getElementById(prefix + 'month').options[document.getElementById(prefix + 'month').selectedIndex].value, 1);
  newDate.setDate(Math.max(1, Math.min(newDate.getDaysInMonth(), input.DateChooser._date.getDate())));
...
function getAbsolutePosition(el) {
  ...
  for (var lx = el.clientWidth+35 , ly = 0; el != null; lx += el.offsetLeft, ly += el.offsetTop, el = el.offsetParent);
...
function DateChooser(input, div, start, end, format, isTimeChooser) {
  ...
  for (var i = 0; i < 10; ++i) this._prefix += letters[Math.floor(Math.random() * letters.length)];
}

DateChooser.prototype._isVisible = false;
DateChooser.prototype.isVisible = function() { return this._isVisible;}
...
DateChooser.prototype.createChooserHtml = function() {
  var formHtml = "<input type=hidden id=" + this._prefix + "inputId value=\"" + this._input.getAttribute('id') + "\">" + "\r\n  <select id=\"" + this._prefix + "month\" onChange=\"dateChooserDateChange( '" + this._prefix + "');\">";
  for (var monIndex = 0; monIndex <= 11; monIndex++) {
    formHtml += "<option value=\"" + monIndex + "\"" + (monIndex == this._date.getMonth() ? " selected=\"1\"" : "") + ">" + Date.monthNames[monIndex] + "</option>";
  }
  formHtml += "</select><select id=" + this._prefix + "year onChange=\"dateChooserDateChange('" + this._prefix + "');\">";
  for (var i = this._start; i <= this._end; ++i) {
    formHtml += "<option value=\"" + i + "\"" + (i == this._date.getFullYear() ? " selected=\"1\"" : "") + ">" + i + "</option>";
  }
  formHtml += "</select>";
...
DateChooser.prototype.createTimeChooserHtml = function() {
  ...
  for (var i = 0; i < 12; ++i) {
    result += "\r\n    <option value=\"" + i + "\"" + (((this._date.getHours() % 12) == i) ? " selected=\"1\">" : ">") + i + "</option>";
  }
  ...
  for (var i = 0; i < 60; i += 15) {
    result += "\r\n    <option value=\"" + i + "\"" + ((this._date.getMinutes() == i) ? " selected=\"1\">" : ">") + String.leftPad(i, 2, '0') + "</option>";
  }
  result += "\r\n  </select>";
  // Add AM/PM
  result += "\r\n  <select name=\"" + this._prefix + "ampm\">";
  result += "\r\n    <option value=\"0\"" + (this._date.getHours() < 12 ? " selected=\"1\">" : ">") + "AM</option>";
  result += "\r\n    <option value=\"12\"" + (this._date.getHours() >= 12 ? " selected=\"1\">" : ">") + "PM</option>";
  result += "\r\n  </select>";
  return result;
}
...
DateChooser.prototype.createCalendarHtml = function() {
  ...
  while (i < this._date.getDaysInMonth()) {
    ...
    this._date.getFullYear(), this._date.getMonth(), i);
    var js = '"dateChooserSetDate(\'' + this._input.getAttribute('id') + "', '" + thisDay.dateFormat(this._format) + '\');"'
    result += "\r\n    <td class=\"dateChooserActive"
    // If the date is the currently chosen date, highlight it
    + (i == this._date.getDate() ? " dateChooserActiveToday" : "") + "\" onClick=" + js + ">" + i + "</td>";
  }
