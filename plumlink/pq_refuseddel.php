...
$pdf = new TCPDF();
 ...
 $txt = breaktolines("An attempted delivery has been rejected for the following reason:",160);
 $msg = str_replace(chr(252),' ',$request['doc'][3]) ;
 $txt = array_merge($txt,breaktolines($msg,160));
 ...
 for ($i = 0;$i < count($txt);$i++) {
  ...
  $pdf->Text(15,$ypos,$txt[$i]);
  $ypos = $ypos + 5;
 }
 lastfooter();

$filename = __DIR__.'/pdfdocs/REFUSEDDEL_'.$request['id'][0].'.PDF';
echo $filename;
...
function firstheader() {
 ...
 $pdf->Text(15,85,'Refusal reasons');
